...
// 👤 USER: GET ALL PRODUCTS (FROM DATABASE)
...
// ✏️ Update profile fields: name, username, description
...
// 🔁 Change role (user <-> seller). Admin role cannot be assigned here.
router.put("/me/role", auth, async (req, res) => {
  try {
    ...
    // sign a fresh token with updated role
    if (!process.env.JWT_SECRET) console.warn("JWT_SECRET not set; token will be invalid")
